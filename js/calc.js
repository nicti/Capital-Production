...
var Archon = [23757,11,10,0,0,3,8,11,0,49,0,11,0,11,7,3,4,11,0,0];
var Chimera = [23915,4,6,0,0,11,6,11,0,41,0,11,0,4,4,11,11,11,0,0];
var Thanatos = [23911,9,8,0,0,8,10,11,0,61,0,11,0,7,8,8,7,11,0,0];
...
function calcOre(Trit, Pye, Mex, Iso,Nocxium,Zydrine,Megacyte){
	...
	newmex = Mex-gne*2400*yield/100;
	newiso = Iso-gne*300*yield/100;
	if (newiso > 0){
		dar = 80*newiso/(1600*yield);
	}else{
		dar = 0
	}
	newnoc = Nocxium-dar*120*yield/100;
	if (newnoc > 0){
	...
	newzyd = Zydrine-cro*135*yield/100;
	if (newzyd > 0){
	...
	newmeg = Megacyte-bis*100*yield/100;
	if (newmeg > 0){
	...
	spo = 0;
	newiso = Iso-gne*300*yield/100-dar*1600*yield/100; 
	newtrit = Trit-ark*22000*yield/100-cro*21000*yield/100-dar*10000*yield/100-spo*56000*yield/100; 
	
	if (newtrit > 0){
		spo = 100*newtrit/(56000*yield);
	}  
	newpye = Pye-bis*12000*yield/100-gne*2200*yield/100-spo*12050*yield/100;
	if (newpye > 0)
	{
	...
	newmex = Mex-gne*2520*yield/100;
	newiso = Iso-gne*315*yield/100;
	if (newiso > 0){
		dar = 80*newiso/(1680*yield);
	}else{
		dar = 0
	}
	newnoc = Nocxium-dar*126*yield/100;
	if (newnoc > 0){
	...
	newzyd = Zydrine-cro*142*yield/100;
	if (newzyd > 0){
	...
	newmeg = Megacyte-bis*105*yield/100;
	if (newmeg > 0){
	...
	spo = 0;
	newiso = Iso-gne*315*yield/100-dar*1680*yield/100; 
	newtrit = Trit-ark*23100*yield/100-cro*22050*yield/100-dar*10500*yield/100-spo*58800*yield/100; 
	
	if (newtrit > 0){
		spo = 100*newtrit/(58800*yield);
	}  
	newpye = Pye-bis*12600*yield/100-gne*2310*yield/100-spo*12653*yield/100;
	if (newpye > 0)
	{
	...
	newmex = Mex-gne*2640*yield/100;
	newiso = Iso-gne*330*yield/100;
	if (newiso > 0){
		dar = 80*newiso/(1760*yield);
	}else{
		dar = 0
	}
	newnoc = Nocxium-dar*132*yield/100;
	if (newnoc > 0){
	...
	newzyd = Zydrine-cro*149*yield/100;
	if (newzyd > 0){
	...
	newmeg = Megacyte-bis*110*yield/100;
	if (newmeg > 0){
	...
	spo = 0;
	newiso = Iso-gne*330*yield/100-dar*1760*yield/100; 
	newtrit = Trit-ark*24200*yield/100-cro*23100*yield/100-dar*11000*yield/100-spo*61600*yield/100; 
	
	if (newtrit > 0){
		spo = 100*newtrit/(61600*yield);
	}  
	newpye = Pye-bis*13200*yield/100-gne*2420*yield/100-spo*13255*yield/100;
	if (newpye > 0)
	{
